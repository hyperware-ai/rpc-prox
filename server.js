...
function closeAllTunnelsOnProviderChange() {
    onProviderChange(({ previous, current, reason }) => {
        ...
        shell.exec(`curl -X POST -H 'Content-Type: application/json' -d '{"file": "server (proxy)", "type": "info", "subject": "info (email notify)", "message": "Provider changed from ${previous} to ${current}${reason ? ` (${reason})` : ''}"}' ${process.env.BACKEND_URL}/raise-alarm`);
